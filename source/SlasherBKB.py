...
class SlasherBKB(Tk):
  def __init__(self):
    ...
    self.split_height = StringVar(value="30000")
    self.enable_pixels_mode = BooleanVar(value=True) #Custom
    self.disable_pixels_mode = BooleanVar(False)  # Custom
    self.pixels_count = StringVar(value="5") #Custom
    ...
    self.width_enforce_type = StringVar(value="Без соблюдения ширины")
    ...
    self.status = StringVar(value="Простой")
    self.num_of_inputs = 1
    self.progress = ""
    self.console_canvas = ""
    self.subprocess_console = ""
    self.width_enforce_types = ['Без соблюдения ширины', 'Автоматическая равномерная ширина', 'Пользовательская ширина']
    # Application StartUp Sequence
    self.load_app_settings()
    self.setup_window(app_maintainer="SlasherBKBConsole", app_version="1.1.1")
  ...
  def save_app_settings(self, *args):
    ...
    app_settings.append(self.enable_pixels_mode.get()) #Custom
    app_settings.append(self.disable_pixels_mode.get())  # Custom
    app_settings.append(self.pixels_count.get()) #Custom
  ...
  def load_app_settings(self):
    ...
    if not os.path.exists("settings.pickle"):
      ...
      with open("settings.pickle", 'rb') as app_settings_handler:
        ...
        self.enable_pixels_mode.set(saved_settings[2]) #Custom
        self.disable_pixels_mode.set(saved_settings[3])  # Custom
        self.pixels_count.set(saved_settings[4]) #Custom
        self.output_files_type.set(saved_settings[5])
        self.show_advanced_settings.set(saved_settings[6])
        self.slicing_senstivity.set(saved_settings[7])
        self.ignorable_edges_pixels.set(saved_settings[8])
        self.width_enforce_type.set(saved_settings[9])
        self.custom_enforce_width.set(saved_settings[10])
        self.show_subprocess_settings.set(saved_settings[11])
        self.enable_subprocess_execution.set(saved_settings[12])
        self.subprocess_path.set(saved_settings[13])
        self.subprocess_arguments.set(saved_settings[14])
        self.last_input_folder = saved_settings[15]
  ...
  def setup_window(self, app_maintainer, app_version):
    ...
    self.title("SlasherBKB" + " [" + app_version + "]")
    self.iconphoto(False, PhotoImage(file="Source/SlasherBKBLogo.png"))
  ...
  def setup_paths_frame(self):
    ...
    paths_frame = ttk.LabelFrame(self, text="Параметры ввода/вывода", padding=(5,0))
    paths_frame.columnconfigure(0, weight=1)
    # Setup of Input Label, Entry Field, and Browse Button.
    input_label = ttk.Label(paths_frame, text = 'Путь ввода')
    input_field = ttk.Entry(paths_frame, textvariable=self.input_folder)
    input_field.bind("<Any-KeyRelease>", self.update_output_path)
    input_button = ttk.Button(paths_frame, text = 'Обзор', command=self.browse_input_path)
    ...
    output_label = ttk.Label(paths_frame, text = 'Путь вывода')
    ...
    batch_checkbox = ttk.Checkbutton(paths_frame, variable=self.enable_batch_mode, text = 'Пакетный режим [Входная папка содержит несколько папок с главами]', command=self.save_app_settings)
  ...
  def setup_basic_settings_frame(self):
    ...
    basic_settings_frame = ttk.LabelFrame(self, text="Базовые настройки", padding=(5,0))
    ...
    pixels_checkbox = ttk.Checkbutton(basic_settings_frame, variable = self.disable_pixels_mode,
                                     text='Высота скана (в пикселях):        ',
                                     command=self.check_disable_pixels)
    ...
    pixels_checkbox.grid(row=0, column=0, columnspan=2, pady=(0,0), sticky="new")
    split_field.grid(row=1, column=0, padx=(4,5), pady=(0,0), sticky="new")
    # Setup number of scans.
    pixels_checkbox = ttk.Checkbutton(basic_settings_frame, variable=self.enable_pixels_mode,
                                     text='Кол-во сканов на выходе:        ',
                                     command=self.check_enable_pixels)
    pixels_field = ttk.Entry(basic_settings_frame, textvariable=self.pixels_count, validate='all')
    pixels_field.bind("<Any-KeyRelease>", self.save_app_settings)
    pixels_field['validatecommand'] = (pixels_field.register(self.validate_nums_only), '%P', '%d', '%s')
    pixels_checkbox.grid(row=0, column=1, columnspan=2, pady=(0, 0), sticky="new")
    pixels_field.grid(row=1, column=1, padx=(4, 5), pady=(0, 0), sticky="new")
    # Setup of Output File Type Settings' Label, and Entry Field.
    type_label = ttk.Label(basic_settings_frame, text='Тип изображений на выходе:')
    ...
    type_label.grid(row=2, column=0, padx=(4,5), pady=(0,0), sticky="new")
    type_dropdown.grid(row=3, column=0, padx=(4,5), pady=(0,5), sticky="new")

    link = ttk.Button(basic_settings_frame, text='Vk: Bikini Bottom', command=self.vklink)
    link.grid(row=3, column=1, columnspan=2, padx=(4,5), pady=(0,5), sticky="new")
  ...
  def setup_advanced_settings_frame(self):
    ...
    shown_settings_frame = ttk.LabelFrame(advanced_settings_frame, text="Расширенные настройки", padding=(5,0))
    ...
    senstivity_label = ttk.Label(shown_settings_frame, text = 'Чувствительность обнаружения пустоты (%):')
    ...
    ignorable_pixels_label = ttk.Label(shown_settings_frame, text = 'Игнорируемые пиксели границ:')
    ...
    scan_line_label = ttk.Label(shown_settings_frame, text = 'Отступ от конца скана:')
    ...
    width_enforce_label = ttk.Label(shown_settings_frame, text = 'Настройка ширины:')
    width_enforce_dropdown = ttk.Combobox(shown_settings_frame, textvariable=self.width_enforce_type, values=('Без соблюдения ширины', 'Автоматическая равномерная ширина', 'Пользовательская ширина'))
    ...
    show_advanced_checkbox = ttk.Checkbutton(advanced_settings_frame, variable=self.show_advanced_settings, text = 'Показать расширенные настройки', command=lambda: self.advanced_settings_toggle(shown_settings_frame, subprocess_setting_frame))
  ...
  def setup_subprocess_frame(self):
    ...
    # Browse Button and Input and Output Field
    subprocess_frame = ttk.LabelFrame(self, text="Настройки подпроцесса", padding=(5,0))
    ...
    show_advanced_checkbox = ttk.Checkbutton(subprocess_frame, variable=self.enable_subprocess_execution, text = 'Запустить следующий подпроцесс после завершения склеивания', command=lambda: self.save_app_settings)
    show_advanced_checkbox.grid(row = 0, column = 0, columnspan=2, pady=(2,5), sticky="new")
    # Setup of Path Label, Entry Field, and Browse Button.
    subprocess_path_label = ttk.Label(subprocess_frame, text = 'Расположение файла подпроцесса / путь')
    subprocess_path_field = ttk.Entry(subprocess_frame, textvariable=self.subprocess_path)
    subprocess_path_button = ttk.Button(subprocess_frame, text = 'Обзор', command=self.browse_subprocess_path)
    ...
    subprocess_arguments_label = ttk.Label(subprocess_frame, text = 'Аргумент подпроцесса')
    ...
    # Setup of Back Mode Selector/Checkbox.
    argument_hint_label = ttk.Label(subprocess_frame, foreground='blue', text = 'Используйте [ Разрезано ] для передачи выходного каталога склейщика, Используйте [ processed ] для передачи выходного каталога пользовательского процесса. ', justify=LEFT, wraplength=400)
  ...
  def setup_console_frame(self, Frame):
    ...
    console_main_frame = ttk.LabelFrame(Frame, text="Консоль подпроцесса")
  ...
  def setup_action_frame(self):
    ...
    action_frame = ttk.LabelFrame(self, text='Текущее состояние', padding=(5,0))
    ...
    self.actionbutton = ttk.Button(action_frame, text = 'ПОПЛЫЛИ', style="Accent.TButton", command=self.run_stitch_process_async)
  ...
  # Custom
  def check_enable_pixels(self):
    if self.enable_pixels_mode.get() == False:
      self.disable_pixels_mode.set(value=True)
    if self.enable_pixels_mode.get() == True:
      self.disable_pixels_mode.set(value=False)
    self.save_app_settings()

  def vklink(self):
    webbrowser.open_new("https://vk.cc/caPtU6")

  # Custom
  def check_disable_pixels(self):
    if self.disable_pixels_mode.get() == False:
      self.enable_pixels_mode.set(value=True)
    if self.disable_pixels_mode.get() == True:
      self.enable_pixels_mode.set(value=False)
    self.save_app_settings()
  ...
  def update_width_mode(self, widthfieldtitle, widthfield):
    ...
    if enforce_type == 'Автоматическая одинаковая ширина':
  ...
  def browse_input_path(self):
    ...
    self.output_folder.set(foldername + " [Разрезано]")
    self.save_app_settings()
  ...
  def update_output_path(self, *args):
    """Opens Browse Dialog and Gets Directory For the Input and Updates Output."""
    self.output_folder.set(self.input_folder.get() + " [Разрезано]")
    self.save_app_settings()
  ...
  def pre_process_check(self):
    ...
    self.actionbutton['state'] = "отключено"
    if(self.input_folder.get() == ""):
      self.status.set("Не указан путь к входной папке!")
      self.actionbutton['state'] = "норма"
      return False
    if(self.output_folder.get() == ""):
      self.status.set("Не задан путь к выходной папке!")
      self.actionbutton['state'] = "норма"
      return False
    if(not os.path.exists(self.input_folder.get())):
      self.status.set("Путь к входной папке не существует.")
      self.actionbutton['state'] = "норма"
      return False
    if(self.split_height.get() == "" or self.split_height.get() == "0"):
      self.status.set("Значение 'Высоты сканов' не было установлено")
      self.actionbutton['state'] = "норма"
      return False
    if (self.pixels_count.get() == "" or self.pixels_count.get() == "0"):
      self.status.set("Значение 'Кол-во сканов' не было установлено!")
      self.actionbutton['state'] = "норма"
      return False
    if(self.slicing_senstivity.get() == ""):
      self.status.set("Значение чувствительности обнаружения не было установлено!")
      self.actionbutton['state'] = "норма"
      return False
    if(self.ignorable_edges_pixels.get() == ""):
      self.status.set("Не установлено значение игнорирвания пикселей")
      self.actionbutton['state'] = "норма"
      return False
    if(self.scan_line_step.get() == ""):
      self.status.set("Не установлено значение шага линии сканирования!")
      self.actionbutton['state'] = "норма"
      return False
    return True
  ...
  def stitch_process(self):
    ...
    self.status.set("Выполнение - Загрузка файлов изображений!")
    ...
    if (self.num_of_inputs == 0):
      return "пакетный режим без папок"
    for path in folder_paths:
      ...
      if width_type_index == 0:
        ...
        self.update_gui_progress("Работа - Изменение Размера и склеивания файлов Изображений!", (10 / self.num_of_inputs))
      ...
      #Custom
      if self.disable_pixels_mode.get() == True:
        self.update_gui_progress("Работа - Нарезка склееного изображения на итоговые изображения!", (10 / self.num_of_inputs))
      ...
      if self.enable_pixels_mode.get() == True:
        self.update_gui_progress("Работа - Нарезка склееного изображения на итоговые изображения!", (10 / self.num_of_inputs))
        final_images = ssc.split_image(combined_image, ssc.new_count_heights(resized_images, self.pixels_count.get()), self.slicing_senstivity.get(),
                                       self.ignorable_edges_pixels.get(), self.scan_line_step.get())
      ...
      if (self.enable_subprocess_execution.get()):
        self.status.set("Рабочий - Запуск подпроцесса над итоговыми изображениями!")
        processed_path = path[1]+ " [Выполнение]"
  ...
  def run_stitch_process(self):
    ...
    try:
      ...
      logging.exception("Возникла непредвиденная ошибка на " + str(datetime.now()))
      process_status = "Краш"
    ending_time = time()
    delta = ending_time - starting_time
    if (process_status == "Краш"):
      self.status.set("Произошла непредвиденная ошибка, проверьте файл 'crashreport.log'.")
    elif (process_status == "Пакетный режим без папок"):
      self.status.set("Включен пакетный режим, не найдено подходящих входных папок!")
    elif (process_status == "Нет изображений"):
      self.status.set("Изображения не найдены")
    else:
      self.status.set("Фаил успешно разрезан за " +  str(delta) + "sec!")
    self.progress['value'] = 0
    self.actionbutton['state'] = "Норма"
